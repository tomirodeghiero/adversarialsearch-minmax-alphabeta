...
angular.module('Tree', ['Enums', 'ActionListQueue'])
  .factory('TreeService', ['EnumService', 'ActionLQService', function(EnumService, ActionLQService) {
    ...
    Tree.prototype.minimax = function() {
      var thisTree = this;
      
      var minimaxActions = function(node, maxNode) {
        var enterActions = [
          new Action(node.edgeToParent, 'entered', false, true),
          new Action(node, 'entered', false, true),
          new Action(node, 'minmax', null, maxNode ? 'MAX' : 'MIN')
        ];
        var childActionsList = [];
        ...
        var childResults = [];
        for (var k = 0; k < node.childNum; k++) {
          var child = node.getKthChild(k);
          var res = minimaxActions(child, !maxNode);
          childResults.push(res);
          childActionsList.push(res.enterActions);
          childActionsList.extend(res.childActionsList);
          childActionsList.push(res.exitActions);
        }
    
        var bestValue = maxNode ? Number.NEGATIVE_INFINITY : Number.POSITIVE_INFINITY;
        for (var i = 0; i < childResults.length; i++) {
          var childValue = childResults[i].returnVal;
          if (maxNode) {
        ...
        var setValueAction = new Action(node, 'value', node.__value, bestValue);
        node.__value = bestValue;
...
angular.module('abTreePractice', ['d3', 'Enums', 'Tree'])
  .controller('MainCtrl', [
    ...
    $scope.flipMax = function() {
      ...
      $scope.generateRootNode();
      $scope.updateMinMaxLabels();
    };
    $scope.updateMinMaxLabels = function() {
      d3.selectAll('.minmax-label').remove();
      addMinMaxLabels(d3.select('svg'), $scope.nodes, $scope.yOffset);
    };

    $scope.checkAnswer = function() {
      if (!$scope.actionLQ) {
        $scope.actionLQ = $scope.useAb ? $scope.tree.alphaBeta() : $scope.tree.minimax();
      }
    ...
    $scope.slider = new Slider('input.slider', {
      ...
      formatter: function(value) {
        return (value / 1000) + 's por acción';
      },
    }).on('slide', function(e) {
      $scope.timeStep = e;
    });
      ...
      link: function(scope, element, attrs) {
        ...
        d3Service.d3().then(function(d3) {
          ...
          scope.renderD3Tree = function() {
            ...
            var xMargin = svgMargin + 50;
            renderD3SubTree(root, xMargin, svgWidth - svgMargin,
                scope.nodes, scope.links);

            addMinMaxLabels(svg, scope.nodes, yOffset);
          ...
          scope.reRender = function() {
            ...
            .classed('pruned', function(d) { 
              return scope.useAb && d.pruned; // Only show pruned edges for alpha-beta
            })
            ...
            .classed('pruned', function(d) { return scope.useAb && d.pruned; });
          ...
          function addMinMaxLabels(svg, nodes, yOffset) {
            var levels = {};
            nodes.forEach(function(node) {
              if (node.nodeType !== TreeNodeTypeEnum.leafNode && !levels[node.depth]) {
                levels[node.depth] = node.nodeType;
              }
            });
          
            Object.keys(levels).forEach(function(depth) {
              svg.append('text')
                .attr('class', 'minmax-label')
                .attr('x', 10)
                .attr('y', topMargin + (yOffset * (depth - 1)))
                .text(levels[depth] === TreeNodeTypeEnum.maxNode ? 'MAX' : 'MIN');
            });
          }
          
          $scope.addMinMaxLabels = addMinMaxLabels;
